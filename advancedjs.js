...
// can also be done using maps

// Nullish coalescing: checks either null or undefined
function calculatePrice(price, tax, description){
  tax = tax ?? 0.05; // either use tax or use the defualt of 0.05 when null or underfined; could have used or symbol but 0 is taken as false so didnt use it
...
function printPersonStreet(person){
  console.log(person?.address?.street); //? defines if the property is presnet then looked into it else print undefined
}

const kyle = new Person ("Kyle", {street: '12 main st', city: 'Ktm'},['bowling','video-gamiing']);
...
const info = {
  name2, // no need to write name2: name2 to make K-V pair; if same name as top obj., JS will automatically think its K-V pair
  favFood
}
